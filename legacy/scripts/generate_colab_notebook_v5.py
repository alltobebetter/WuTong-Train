...
# -*- coding: utf-8 -*-
"""
生成 V5 Colab 训练 Notebook（外部数据集整合版）
"""
...
def generate_v5_notebook():
    ...
    # Cell 8: 下载并整合 CSIC 2010
    notebook["cells"].append({
        "cell_type": "code",
        "metadata": {"id": "csic2010"},
        "source": [
            "# 7. 下载并整合 CSIC 2010 数据集（V5 核心）\n",
            "print(\"=\"*60)\n",
...
def main():
    """主函数"""
    notebook = generate_v5_notebook()
    
    output_path = Path(__file__).parent.parent / "jupyter" / "colab_train_v5.ipynb"
    output_path.parent.mkdir(exist_ok=True)
    
    with open(output_path, "w", encoding="utf-8") as f:
        json.dump(notebook, f, ensure_ascii=False, indent=2)
    
    print(f"V5 Notebook 已生成: {output_path}")
    print("可以上传到 Google Colab 进行训练")
    print(f"\n特点:")
    print(f"  - 自动下载并整合 CSIC 2010 数据集")
    print(f"  - SMOTE 过采样 + XGBoost 优化")
    print("  - 预期准确率: 99.8%+")
